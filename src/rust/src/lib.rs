...
fn check_pkcs7_padding(data: &[u8], block_len: u8) -> bool {
    let mut mismatch = 0;
    let pad_size = data[(block_len - 1) as usize];
    for i in 0..block_len {
        let mask = constant_time_lt(i, pad_size);
        let b = data[(block_len - 1 - i) as usize] as u8;
        mismatch |= mask & (pad_size ^ b);
    }

    // Check to make sure the pad_size was within the valid range.
    mismatch |= !constant_time_lt(0, pad_size);
    mismatch |= constant_time_lt(block_len, pad_size);
...
#[pyo3::prelude::pyfunction]
fn check_ansix923_padding(data: &[u8], block_len: u8) -> bool {
    let mut mismatch = 0;
    let pad_size = data[(block_len - 1) as usize];
    // Skip the first one with the pad size
    for i in 1..block_len {
        let mask = constant_time_lt(i, pad_size);
        let b = data[(block_len - 1 - i) as usize];
        mismatch |= mask & b;
    }
